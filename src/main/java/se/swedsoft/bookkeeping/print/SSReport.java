...
/**
 * @version $Id: SSReport.java 243 2025-06-26 13:14:31Z ellefj $
 *
 */
public class SSReport {
    ...
    private void compileDesign() throws SSException {
        ...
        if (iFields.containsKey(ReportField.LAST_PAGE_FOOTER)) {
            iLastPageFooter = getField(ReportField.LAST_PAGE_FOOTER, theFields,
                    theParameters, theVariables, theGroups);
        }

        iDesign = new JasperDesign();
        iDesign.setResourceBundle("book");
        iDesign.setName("JasperDocument");
        iDesign.setWhenNoDataType(WhenNoDataTypeEnum.NO_DATA_SECTION); // JRReport.WHEN_NO_DATA_TYPE_NO_PAGES
        ...
        ((JRDesignSection)iDesign.getDetailSection()).addBand(iDetail);
        ...
        try {
            ...
            for (JRStyle iStyle: cReportCache.getStyleTemplate().getStyles()){
                try {
     ...
     * @throws SSException
     */
    private JRBand getField(ReportField pField, List<JRField> pFields, List<JRParameter> pParameters, List<JRVariable> pVariables, List<JRGroup> pGroups) throws SSException {

        String pReportName = iFields.get(pField);

        try {
            // Get the report
            JasperDesign iReport = cReportCache.getReport(pReportName);
            ...
            case DETAIL:
                return iReport.getDetailSection().getBands()[0];
    ...
    private JasperPrint getNoPagesReport() {
        ...
        iDesign.setWhenNoDataType(WhenNoDataTypeEnum.NO_DATA_SECTION);
